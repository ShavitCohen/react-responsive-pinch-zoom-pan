...
const SizedContainerView = ({menu, width, height}) => {
  ...
  const handleChange = ({scale, left, top}) => {
    setTop(top);
    setLeft(left);
    setScale(scale);
  };
...
const FlexContainerView = ({menu}) => (
  <div style={{height: '100vh', display: 'flex', flexDirection: 'column'}}>
    ...
    <div style={{flex: 'none', textAlign: 'center'}}>
      <span style={flexContentStyle}>
        The image fills 100% of the flex item in which it is contained
      </span>
    </div>
...
const Menu = ({viewId, onViewChange}) => {
  const getLinkStyle = linkViewId => {
    return {
      padding: 10,
      color: viewId === linkViewId ? 'orange' : 'blue',
    };
  };
  ...
  return (
    <React.Fragment>
      ...
      <a href="#" onClick={() => onViewChange(0)} style={getLinkStyle(0)}>
        Small
      </a>
      <a href="#" onClick={() => onViewChange(1)} style={getLinkStyle(1)}>
        Medium
      </a>
      <a href="#" onClick={() => onViewChange(3)} style={getLinkStyle(3)}>
        Centered
      </a>
      <a href="#" onClick={() => onViewChange(2)} style={getLinkStyle(2)}>
        Full-screen Flex
      </a>
    </React.Fragment>
  );
...
class App extends React.Component {
  ...
  render() {
    ...
    return viewId === 2 ? (
      ...
      <SizedContainerView menu={menu} width={500} height={800} />
    ) : (
