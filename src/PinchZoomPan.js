...
export default class PinchZoomPan extends React.Component {
    ...
    if (event.touches.length === 0 && event.changedTouches.length === 1) {
      if (
        ...
        const pointerPosition = getRelativePosition(
          ...
          this.imageRef.parentNode.parentNode
        );
  ...
  handleMouseDoubleClick = event => {
    this.cancelAnimation();
    var pointerPosition = getRelativePosition(
      event,
      this.imageRef.parentNode.parentNode
    );
    this.doubleClick(pointerPosition);
  ...
  handleImageLoad = event => {
    ...
    const {scale, top, left} = this.props;
    this.setState({scale, top, left});
  ...
  constrainAndApplyTransform(
    ...
    const {scale, top, left} = this.state;
    this.props.onChange({scale, left, top});
